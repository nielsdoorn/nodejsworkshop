...
\rhead{Node.js Workshop}
\lhead{Enschede Web Developers Meetup}
\chead{}
...
Node is not a framework nor a programming language. Programs running on Node are written in JavaScript, the same JavaScript you can use on webpages except that it doesn't run in your browser but on the Chrome JavaScript engine. Therefore, you could call Node.js a platform.

You can run Node on your Mac, PC or server, in the cloud at Nodejitsu and Heroku but also on a Raspberry Pi\footnote{for example to control a quadcopter}. 
...
You can avoid this by using callbacks. Callbacks are references to functions that can be executed at a later time, for example when the database query is complete.
...
\noindent If you write programs in Node that depend on modules it is a good idea to include a \texttt{package.json} file that lists the dependencies.
\begin{lstlisting}
{
...
When someone downloads your program, you can install all dependencies by running:
\begin{lstlisting}[language=bash]
npm install
\end{lstlisting}
and npm will install all required modules.
...
Hosting can be done in the cloud at \href{https://www.nodejitsu.com/}{Nodejitsu}, free for the thirty days, and at \href{https://www.heroku.com/}{Heroku}, free for one instance. Both are easy to work with once you got the hang of it. Heroku doesn't yet support websockets whereas Nodejitsu does.
...
\noindent And the HTML to tie them together:
...
First we'll need to create a \texttt{package.json} file with the dependencies:
\lstinputlisting[language=JavaScript]{code/weather/package.json}
\noindent Then we can install the required modules and their dependencies:
\begin{lstlisting}[language=bash]
npm install
\end{lstlisting}

\noindent To avoid hardcoding the API key in the key we use the environment to set it, in Mac/os and Linux this can be done in the terminal like so:\footnote{In Windows you'll have to do complicated things with system settings (right click on `My Computer', `select properties', open the `advanced' tab, select `Environmental variables', choose new, enter a name `FORECAST\_API\_KEY' and a value: your API key).}
\begin{lstlisting}[language=bash]
export FORECAST_API_KEY=dd987b047af59deaed4a2e2f55d21a0
\end{lstlisting}

\noindent Then we can start using the forecast.io API for example by asking the weather for a specific location. In the next example the weather is requested and the summary is retreived from the response.
...
\begin{itemize}
	\item twitter hashtag subscription
	\item lastfm search
	\item instagram integration
	\item dropbox? https://github.com/sintaxi/node-dbox
	\item foursquare? https://developer.foursquare.com/overview/realtime
\end{itemize}
